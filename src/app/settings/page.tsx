    ...
    <div className="flex-1 flex flex-col overflow-y-auto">
      <div className="flex-1 max-w-2xl w-full mx-auto px-6 py-8 flex flex-col gap-6 text-left">
        <div>
          ...
          <p className="text-sm text-text opacity-50 mt-1">
        ...
        <section className="bg-bg border border-text/10 rounded-2xl overflow-hidden">
          <div className="px-5 py-4 border-b border-text/10">
            <h2 className="text-sm font-semibold uppercase tracking-widest text-text opacity-40">
              Account
            </h2>
          </div>
        ...
        <section className="bg-bg border border-text/10 rounded-2xl overflow-hidden">
          ...
          <div className="px-5 py-4 flex flex-col gap-4">
            <div className="flex items-center justify-between">
              ...
              <StatusBadge enabled={hasPassword} />
            </div>
          ...
          <div className="px-5 py-4 flex flex-col gap-3">
            {error === "email_mismatch" && (
              <div className="text-sm text-red-500 bg-red-500/10 border border-red-500/20 rounded-lg px-4 py-3">
                Your GitHub email doesnt match your account email. Please use a
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                {/* GitHub icon */}
                <svg
                  className="w-5 h-5 text-text opacity-70"
                  fill="currentColor"
                  viewBox="0 0 24 24"
                >
              ...
              <StatusBadge enabled={!!hasGithub} />
            </div>
            {!hasGithub && (
              <p className="text-sm text-text opacity-60 m">
                To link GitHub, sign in with a GitHub account that uses the same
...
function Field({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-center justify-between">
      <span className="text-sm text-text opacity-60">{label}</span>
    ...
    <span
      className={`text-xs font-medium px-2.5 py-1 rounded-full ${
        enabled
          ...
          : "bg-text/5 text-text opacity-40"
      }`}
    >
      {enabled ? "Connected" : "Not set"}
    </span>
  );
}
